...
- The `partials` folder contains handlebars partials that can be loaded in with a `{{> partial_name}}` template. These partials can be used in either `content` files or directly in root level `.hbs` files.
